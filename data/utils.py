...
def build_adj(boundaries: List[Dict], coboundaries: List[Dict], id_maps: List[Dict], complex_dim: int, include_down_adj: bool):
...
def compute_clique_complex_with_gudhi(x: Tensor,
                                      edge_index: Adj,
                                      size: int,
                                      expansion_dim: int = 2,
                                      y: Tensor = None,
    ...
    shared_boundaries, shared_coboundaries, lower_idx, upper_idx = build_adj(boundaries, co_boundaries, id_maps, complex_dim,
                                                                             include_down_adj)
...
def convert_graph_dataset_with_gudhi(dataset, expansion_dim: int, include_down_adj=True, init_method: str = 'sum'):
    ...
    for data in tqdm(dataset):
        complex = compute_clique_complex_with_gudhi(data.x,
                                                    data.edge_index,
                                                    data.num_nodes,
...
def convert_graph_dataset_with_rings(dataset,
    ...
    complexes = parallel(
        delayed(compute_ring_2complex)(maybe_convert_to_numpy(data.x),
    ...
    for c, complex in enumerate(complexes):
        ...
        if complex.y is None:
            ...
            mask = ~torch.isnan(complex.y)  # In some ogbg-mol* datasets we may have null targets.
            assert torch.equal(complex.y[mask], graph.y[mask])
