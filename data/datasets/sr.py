...
class SRDataset(InMemorySSTreeDataset):
    """A dataset of Strongly Regular graphs."""
    def __init__(self,
                 root,
                 name,
                 max_dim=2,
        ...
        c2tree = (max_ring_size is not None)
        if c2tree:
            assert max_dim == 2
    ...
    def processed_dir(self):
        ...
        suffix = f"_{self._max_ring_size}rings" if self._c2tree else ""
        suffix += f"_down_adj" if self.include_down_adj else ""
        return directory + suffix

    @property
    def processed_file_names(self):
        return ['{}_sstrees_list.pt'.format(self.name)]
    ...
    def process(self):
        ...
        if self._c2tree:
            print(f"Converting the {self.name} dataset to a SSTree...")
            sstrees, max_dim, num_features = convert_graph_dataset_with_rings(graphs,
            ...
            sstrees, max_dim, num_features = convert_graph_dataset_with_gudhi(graphs,
        ...
        torch.save(self.collate(sstrees, self.max_dim), path)
