...
from data.datasets import InMemorySSTreeDataset
from torch_geometric.datasets import ZINC


class ZincDataset(InMemorySSTreeDataset):
    ...
    def __init__(self,
        ...
        self._subset = subset
        self._n_jobs = n_jobs
        super(ZincDataset, self).__init__(root, transform, pre_transform, pre_filter, max_dim=2, c2tree=True, num_classes=1)
    ...
    def processed_file_names(self):
        name = self.name
        return [f'{name}_sstrees.pt', f'{name}_idx.pt']
    ...
    def process(self):
        ...
        print(f"Processing SSTree dataset for {self.name}")
        ...
        idx = []
        start = 0
        print("Converting the train dataset to a SSTree...")
        train_sstrees, _, _ = convert_graph_dataset_with_rings(train_data,
                                                               max_ring_size=self._max_ring_size,
                                                               include_down_adj=self.include_down_adj,
                                                               init_edges=self._use_edge_features,
                                                               init_rings=False,
                                                               n_jobs=self._n_jobs)
        data_list += train_sstrees
        idx.append(list(range(start, len(data_list))))
        start = len(data_list)
        print("Converting the validation dataset to a SSTree...")
        val_sstrees, _, _ = convert_graph_dataset_with_rings(val_data,
                                                             max_ring_size=self._max_ring_size,
                                                             include_down_adj=self.include_down_adj,
                                                             init_edges=self._use_edge_features,
                                                             init_rings=False,
                                                             n_jobs=self._n_jobs)
        data_list += val_sstrees
        idx.append(list(range(start, len(data_list))))
        start = len(data_list)
        print("Converting the test dataset to a SSTree...")
        test_sstrees, _, _ = convert_graph_dataset_with_rings(test_data,
                                                              max_ring_size=self._max_ring_size,
                                                              include_down_adj=self.include_down_adj,
                                                              init_edges=self._use_edge_features,
                                                              init_rings=False,
                                                              n_jobs=self._n_jobs)
        data_list += test_sstrees
        idx.append(list(range(start, len(data_list))))
    ...
    def processed_dir(self):
        ...
        suffix1 = f"_{self._max_ring_size}rings" if self._c2tree else ""
