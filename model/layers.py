...
from data.sstree import MPParams
from torch_geometric.nn.inits import reset
from torch.nn import Linear, Dropout, Sequential, BatchNorm1d as BN
from ogb.graphproppred.mol_encoder import AtomEncoder, BondEncoder
from abc import ABC, abstractmethod

from model.mp_tool import create_fb_weight, create_sb_weight
from typing import Callable, Optional, Tuple


class GCNCModule(torch.nn.Module):
    def __init__(self,
                 dim: int,
                 up_msg_size: int,
                 down_msg_size: int,
                 ...
                 eps: float = 0.,
                 args=None):
        super(GCNCModule, self).__init__()
        ...
        self.fb_weight = create_fb_weight(up_msg_size, args)
        self.sb_weight = create_sb_weight(args.sb_cal)
        self.drop = Dropout(args.mp_drop) if args.mp_drop > 0 else lambda x: x
    ...
    def forward(self, cochain: MPParams):
        x = cochain.x
        ...
        if up_attr is not None:
            ...
            w = self.fb_weight(x[cochain.up_x_i_idx], up_x_j, up_attr, out)
            w = self.drop(w)
            out = torch.mul(w, out)
            out_up = self.aggregate_up(out, cochain)
        else:
            out_up = torch.zeros(x.size(0), self.up_msg_size, device=x.device)

        if boundary_attr is not None:
            out = self.msg_boundaries_nn(boundary_attr)
            out_boundaries = self.aggregate_boundary(out, cochain)
        else:
            out_boundaries = torch.zeros(x.size(0), self.boundary_msg_size, device=x.device)
    ...
    def aggregate_up(self, msg_up: Tensor, cochain: MPParams) -> Tensor:
        ...
        d_out_up = torch.matmul(w.unsqueeze(2), d_up_attr).squeeze(2)  # bij,bijf->bif
        return d_out_up[cochain.x_idx]

    def aggregate_boundary(self, msg_boundary: Tensor, cochain: MPParams) -> Tensor:
        ...
        d_out_boundaries = torch.matmul(w, d_boundary)  # bij,bjf->bif
...
class GCNCConv(torch.nn.Module):
    def __init__(self,
                 up_msg_size: int,
                 down_msg_size: int,
                 boundary_msg_size: Optional[int],
    ...
    def forward(self, *cochain_params: MPParams, start_to_process=0):
...
class GCNEConv(torch.nn.Module):
    def __init__(self,
                 passed_msg_node_nn: Optional[Callable],
                 passed_msg_edge_nn: Optional[Callable],
                 passed_update_node_nn: Optional[Callable],
                 passed_update_edge_nn: Optional[Callable],
                 hidden: int = 32,
                 act_module=torch.nn.ReLU,
                 layer_dim: int = 32,
                 graph_norm=BN,
    ...
    def forward(self, *cochain_params: MPParams):
        ...
        w = self.fb_weight(n_up_x_i, n_up_x_j, n_up_attr, out_n)
        w = self.drop(w)
    ...
    def aggregate_node(self, msg_up: Tensor, cochain: MPParams) -> Tensor:
        w = self.sb_weight(cochain.up_adj)
        d_up_attr = get_dense_up_attr(msg_up, w, (cochain.up_index[0], cochain.up_index[1], cochain.up_index[2]))
        d_out_up = torch.matmul(w.unsqueeze(2), d_up_attr).squeeze(2)  # bij,bijf->bif
        return d_out_up[cochain.x_idx]

    def aggregate_edge(self, msg_boundary: Tensor, cochain: MPParams) -> Tensor:
        w = self.sb_weight(cochain.boundary_adj)
        d_boundary = get_dense_x(msg_boundary, cochain.boundary_attr_mask, cochain.boundary_attr_idx)
        d_out_boundaries = torch.matmul(w, d_boundary)  # bij,bjf->bif
        return d_out_boundaries[cochain.x_idx]
...
class InitReduceConv(torch.nn.Module):
    def __init__(self, reduce='add'):
        """
        Args:
            reduce (str): sum, add, mean, min, max
        """
...
class AbstractEmbedVEWithReduce(torch.nn.Module, ABC):
    def __init__(self, v_embed_layer: Callable, e_embed_layer: Optional[Callable], init_reduce: InitReduceConv):
        ...
        Args:
            ...
            init_reduce: Layer to initialise the cycle features and potentially the edge features.
        """
