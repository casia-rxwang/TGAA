...
class CINCochainConv(CochainMessagePassing):
    ...
    def forward(self, cochain: CochainMessagePassingParams):
        out_up, out_down, _ = self.propagate(cochain.up_index,
                                             cochain.down_index,
                                             None,
                                             x=cochain.x,
...
class CINConv(torch.nn.Module):
    def __init__(self,
        ...
        for dim in range(max_dim + 1):
            mp = CINCochainConv(up_msg_size, down_msg_size, msg_up_nn, msg_down_nn, update_nn, eps, train_eps)
...
class SparseCINCochainConv(CochainMessagePassing):
    ...
    def __init__(self,
                 ...
                 eps: float = 0.,
                 train_eps: bool = False):
        super(SparseCINCochainConv, self).__init__(up_msg_size,
                                                   down_msg_size,
    ...
    def forward(self, cochain: CochainMessagePassingParams):
        out_up, _, out_boundaries = self.propagate(cochain.up_index,
                                                   cochain.down_index,
    ...
    def __init__(self,
        ...
        for dim in range(max_dim + 1):
            ...
            if msg_up_nn is None:
                if use_coboundaries:
                    msg_up_nn = Sequential(Catter(), Linear(kwargs['layer_dim'] * 2, kwargs['layer_dim']),
                                           kwargs['act_module']())
                else:
            ...
            if update_up_nn is None:
                update_up_nn = Sequential(Linear(kwargs['layer_dim'], kwargs['hidden']), graph_norm(kwargs['hidden']),
                                          kwargs['act_module'](), Linear(kwargs['hidden'], kwargs['hidden']),
                                          graph_norm(kwargs['hidden']), kwargs['act_module']())

            update_boundaries_nn = passed_update_boundaries_nn
            if update_boundaries_nn is None:
                update_boundaries_nn = Sequential(Linear(kwargs['layer_dim'], kwargs['hidden']), graph_norm(kwargs['hidden']),
                                                  kwargs['act_module'](), Linear(kwargs['hidden'], kwargs['hidden']),
                                                  graph_norm(kwargs['hidden']), kwargs['act_module']())
            combine_nn = Sequential(Linear(kwargs['hidden'] * 2, kwargs['hidden']), graph_norm(kwargs['hidden']),
                                    kwargs['act_module']())

            mp = SparseCINCochainConv(dim,
                                      up_msg_size,
                                      down_msg_size,
                                      boundary_msg_size=boundary_msg_size,
                                      msg_up_nn=msg_up_nn,
